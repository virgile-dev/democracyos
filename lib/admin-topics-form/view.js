...
export default class TopicForm extends FormView {
  ...
  constructor(topic, forum, forums, tags) {
    ...
    this.previewButton = this.find('a.preview');
    this.viewButton = this.find('a.view');

    var body = this.find('textarea[name=body]');
    this.richtext = new Richtext(body, 'topic', topic ? topic.id : null);
    this.renderToggles();
  }
  ...
  onmakepublicclick(ev) {
    ...
    this.previewButton.addClass('disabled');

    topicStore
      .publish(this.topic.id)
      .then(() => {
        view.pubButton.removeClass('disabled').addClass('hide');
        view.previewButton.removeClass('disabled').addClass('hide');
        view.privButton.removeClass('hide');
        view.viewButton.removeClass('hide');
      })
      .catch(err => {
        view.pubButton.removeClass('disabled');
        view.previewButton.removeClass('disabled');
  ...
  onmakeprivateclick(ev) {
    ...
    topicStore
      ...
      .then(() => {
        ...
        view.viewButton.removeClass('disabled');
        view.viewButton.addClass('hide');
        view.pubButton.removeClass('hide');
        view.previewButton.removeClass('hide');
      })
      .catch(err => {
        view.pubButton.removeClass('disabled');
        view.viewButton.removeClass('disabled');
