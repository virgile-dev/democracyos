...
function signin(req, res, next) {
  ...
  auth(email, req.body.password, function (err, user, info) {
    if (err) {
      return res.json(500, { error: t(err.message) });
    }
    if (!user) {
      return res.json(200, { error: 'signin.error.login-failed' });
    }
    if (!user.emailValidated) {
      return res.json(200, { error: 'signin.error.email-not-valid' });
    }
    if ('disabled' === user.status) {
      return res.json(200, { error: 'signin.account-disabled' });
    }
