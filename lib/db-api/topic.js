...
exports.all = function all(params, fn) {
  ...
  if (params.forum && ('all' !== params.forum)) {
    query.forum = params.forum;
  }

  Topic
  .find(query)
  //.select('id topicId mediaTitle tag participants clauses votes createdAt updatedAt closingAt publishedAt deletedAt status open closed links author authorUrl forum')
  .populate('tag', 'id hash name color image')
  .populate('forum', 'id name title summary image imageUrl')
...
exports.rss = function all(fn) {
  ...
  Topic
  .find({ deletedAt: null })
  .select('id topicId mediaTitle publishedAt clauses content')
  .exec(function (err, topics) {
    if (err) {
      log('Found error %j', err);
      return fn(err);
