...
var fields = 'id firstName lastName fullName email staff roles status profilePictureUrl';
...
exports.create = function create(data, fn) {
  ...
  comment.save(function (err) {
    ...
    User.populate(comment, { path: 'author' }, function(err) {
      ...
      var eventName = 'comment-published';
      var topicUrl = '';
      Topic.get(data.context === 'topic' ? comment.reference : data.topicId , function(err, topic) {
        if (err) return _handleError(err, fn);

        if (topic.forum) {
          topicUrl = utils.buildUrl(config, { pathname: config.subPath + '/' + topic.forum.name + '/topic/' + comment.reference });
        } else {
          topicUrl = utils.buildUrl(config, { pathname: config.subPath + '/topic/' + comment.reference });
        }
        ...
        notifier.notify(eventName)
          ...
          .send(function (err, data) {
            if (err) {
              ...
              log('Successfully notified publishing of comment %s', comment.id);
            }
          });
      });
