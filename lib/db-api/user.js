...
exports.search = function search(query, fn) {
  ...
  .select('firstName lastName fullName email avatar profilePictureUrl notifications roles activities location age')
  .exec(function(err, users) {
    if (err) {
      log('Found error: %j', err);
      return fn(err);
...
exports.getByEmail = function search(email, fn) {
  ...
  .select('firstName lastName fullName email avatar profilePictureUrl notifications emailValidated roles activities location age')
  .exec(function(err, user) {
    if (err) {
...
exports.lookup = function lookup(ids, fn) {
  ...
  .select('firstName lastName fullName email avatar staff status profilePictureUrl notifications roles activities location age')
  .exec(function(err, users) {
...
exports.expose.confidential = function(user){
  return expose('id firstName lastName fullName displayName hasPassword profileType email roles activities location age avatar profilePictureUrl staff status notifications locale')(user);
}
...
exports.expose.ordinary = function(user){
  return expose('id fullName displayName avatar profilePictureUrl profileType email roles activities location age staff status notifications locale')(user);
}
