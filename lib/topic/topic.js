...
import { findForum, findAll as findAllForums } from '../forum-middlewares/forum-middlewares';
...
export function show (ctx) {
  ...
  let comments = new Comments(commentsOptions);
  ...
  function pagechange(url) {
    ...
    // if we still are in a browsing topic page
    if( 0 < url.search('/topic') ) return;

    o(document.body).removeClass('browser-page');
  }
}

page(forumRouter('/topic/:id'), user.optional, findForum, findTopics, findTopic, rules.middleware, (ctx, next) => {
