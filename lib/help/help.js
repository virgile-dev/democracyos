...
import LP from '../help-legal/view.js';
...
page('/help/:page?', valid, parse, (ctx, next) => {
  ...
  if('markdown' !== page ){
    let menuItem = o('.help-menu .' + page + '', container);
    menuItem.parent().addClass('active');
  }

  // Set page's title
  title(o('[href="#{config.subPath}/help/' + page + '"]').text());
  ...
  if(config.legalTerms) lp.appendTo(content);
...
function valid(ctx, next) {
  ...
  if (config.legalTerms) valid.push('legal-terms');
