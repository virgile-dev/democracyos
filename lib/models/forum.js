  ...
  summary: {
    ...
    validate: {
      validator: function(summary) {
        summary = summary || '';
        return config.forumSummaryLength > summary.length;
      },
...
/*
ForumSchema.pre('save', validateOwnerUniqueness);

function validateOwnerUniqueness (next) {
  var self = this;
  this.constructor
  .where({ deletedAt: null, owner: this.owner })
  .exec(function(err, forums) {
    if (err) return next(err);
    if (1 === forums.length && self._id.equals(forums[0]._id)) return next();
    if (forums.length) return next(new Error('User already has a Forum.'));
    next();
  });
}
*/
...
ForumSchema.methods.isAdmin = function (user) {
  ...
  return user.isStaff || ownerId.equals(user._id);
};

ForumSchema.virtual('image').get(function() {
  if( this.imageUrl ){
    return this.imageUrl ;
  } else {
    var data = geopattern.generate(this.id) ;
