...
var aliasValidator = [
  { validator: maxLabelValidator, msg: 'alias.limited' }
];
...
var CommentReplySchema = new Schema({
  ...
  alias: {type: String, maxlength: 110, trim: true },
...
function maxTextValidator(text) {
  return 4096 >= text.length ;
}

function maxLabelValidator(label) {
  return 110 <= label.length ;
}
...
CommentSchema.index({ upvote: -1 });
CommentSchema.index({ downvote: -1 });
...
CommentSchema.methods.vote = function(user, value, cb) {
  ...
  this.upvote = this.upvotes.length;
  this.downvote = this.downvotes.length;
  this.save(cb);
};
