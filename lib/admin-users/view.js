...
export default class UsersListView extends View {
  ...
  switchOn() {
    this.list = new List('users-wrapper', { valueNames: ['lastName', 'fullName', 'createdAt', 'email', 'admin', 'member', 'staff', 'id'] });


    this.searchForm = this.find('input.search');
    this.bind('click', '.filter.staff', this.bound('filterByStaff'));
    this.bind('click', '.filter.elected', this.bound('filterByAdmin'));
    this.bind('click', '.filter.member', this.bound('filterByMember'));

    this.bind('click', '.f-status', this.bound('toggleStatus'));
    this.bind('click', '.f-member', this.bound('toggleAdmin'));
    this.bind('click', '.f-admin', this.bound('toggleMember'));
  ...
  filterByAdmin(){
    this.filter(t('settings.role.admin'));
  }

  filterByMember(){
    this.filter(t('settings.role.member'));
  }
  ...
  toggleAdmin(ev){
    this.toggleRole(ev, 'admin');
  }

  toggleMember(ev){
    this.toggleRole(ev, 'member');
  }
