...
export default class UsersListView extends View {
  ...
  toggle(ev){
    ...
    console.log('this.options.roles = %O',this.options.roles);
    console.log('contains %s', options.filter(options.roles,{name:toggleType}).length);

    if( 'status' === toggleType ){
       user.status = 'active' === user.status ? 'disabled' : 'active';
       console.log('user.status %O',user.status);
    } else if ( options.filter(options.roles,{name:toggleType}).length ) {
      if(0 <= user.roles.indexOf(toggleType)){
        console.log(user.roles.indexOf(toggleType));
