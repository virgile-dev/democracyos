...
app.get('/comment/all', requireTopicId, function (req, res) {
  ...
  var sort = '';
  if (~['-score', '-createdAt', 'createdAt', '-upvote', '-downvote'].indexOf(req.query.sort)) {
    sort = req.query.sort;
  } else {
    sort = '-upvote';
  }
  ...
  api.comment.all({
    ...
    if (!req.query.count) {
      ...
      var keys = [
        ...
        'flags upvote downvote upvotes downvotes votes replies.length'
      ].join(' ');
...
app.get('/comment/mine', restrict, requireTopicId, function (req, res) {
  ...
  log('query sort %o',req.query.sort);
...
app.post('/comment', restrict, requireTopicId, function (req, res) {
  ...
  api.comment.create(comment, function (err, commentDoc) {
    ...
    var keys = [
      'id alias count text',
      'author.id author.fullName author.displayName author.avatar author.staff author.roles author.status',
      'upvote downvote upvotes downvotes flags',
...
app.post('/comment/:id/reply', restrict, function (req, res) {
  ...
  api.comment.reply(req.params.id, reply, function (err, replyDoc) {
    ...
    var keys = [
      'id createdAt text',
      'author.id author.fullName author.avatar author.staff author.roles author.status'
    ].join(' ');
...
app.get('/comment/:id/replies', function (req, res) {
  ...
  api.comment.replies(req.params.id, function(err, replies) {
    ...
    var keys = [
      'id alias count createdAt editedAt text',
      'author.id author.fullName author.avatar author.staff author.roles author.status'
    ].join(' ');
