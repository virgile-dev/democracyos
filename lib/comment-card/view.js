...
export default class CommentCard extends View {
  ...
  switchOn () {
    this.bind('click', '.flag-comment', 'onflag');
    this.bind('click', '.view-options', 'onedit');
  ...
  onflag (ev) {
    ...
    let target = this.find('.flag-comment');
  ...
  onsuccessedit (data) {
    ...
    this.find('.edit-comment').removeClass('hide');
    this.mediaBody.removeClass('edit');
  ...
  onreplyclick (ev) {
    ...
    console.log('comment card onreplyclick');
