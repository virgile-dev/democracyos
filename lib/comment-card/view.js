...
export default class CommentCard extends View {
  ...
  onreplyclick (ev) {
    ...
    if (repliesContainer.hasClass('no-hide')) {
      let commentsRepliesView = this.commentsRepliesView = new CommentsRepliesView(this.comment, this.locals);
   ...
   */
   ...
   toggleReplyForm (ev) {
     ...
     if(this.commentsRepliesView) {
       ...
       this.commentsRepliesView.find('.reply-form')[0].scrollIntoView(false);
     }
   }
