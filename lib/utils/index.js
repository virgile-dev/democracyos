...
exports.buildUrl = function buildUrl(config, options) {
  ...
  log('buildUrl computed options are %j',options);

  return url.format(options);
};

function inferPort(config, options) {
  if(config.auth.exposedPort) return config.auth.exposedPort ;
  else return 'https' === options.protocol ? config.https.port : config.publicPort;
}
