...
config.mongoUrl = env.MONGODB_URI || env.MONGO_URL || env.MONGOLAB_URI || config.mongoUrl;
...
if ('amazon' === config.uploader) {

  if (env.AWS_ACCESS_KEY && env.AWS_SECRET_KEY && env.S3_BUCKET ) {
    config.amazon.username = env.AWS_ACCESS_KEY;
    config.amazon.password = env.AWS_SECRET_KEY;
    config.amazon.bucket = env.S3_BUCKET;
  } else if (!(config.amazon.username && config.amazon.password && config.amazon.bucket)) {
