...
export default class TagForm extends FormView {
  ...
  onsuccess() {
    console.log('tag form onsuccess');
    this.onsave();
  }
  ...
  onimageclick(ev) {
    ...
    this.find('.tag-image.active').removeClass('active');
    jQuery(ev.target).parent('.tag-image').addClass('active');
  }
}
